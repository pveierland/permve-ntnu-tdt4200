...
AccurateImage*
convertImageToNewFormat(PPMImage* image)
{
    // Make a copy
...
AccurateImage*
createEmptyImage(PPMImage* image)
{
    AccurateImage* imageAccurate;
    imageAccurate = (AccurateImage*)malloc(sizeof(AccurateImage));
    imageAccurate->data =
...
void
performNewIdeaIteration(AccurateImage* imageOut,
                        ...
                        int size)
{
    ...
    AccuratePixel* line_buffer =
        (AccuratePixel*)malloc(imageIn->x * sizeof(AccuratePixel));
    memset(line_buffer, 0, imageIn->x * sizeof(AccuratePixel));

    // Iterate over each line of pixelx.
    for (int senterY = 0; senterY < imageIn->y; senterY++)
    {
        // first and last line considered  by the computation of the pixel in
        // the line senterY
        ...
        if (starty <= 0)
        {
            ...
            if (senterY == 0)
            {
                // for all pixel in the first line, we sum all pixel of the
                // column (until the line endy)
                ...
                for (int line_y = starty; line_y < endy; line_y++)
                {
                    ...
                    {
                        line_buffer[i].blue +=
                            imageIn->data[numberOfValuesInEachRow * line_y + i]
                                .blue;
                        line_buffer[i].red +=
                            imageIn->data[numberOfValuesInEachRow * line_y + i]
                                .red;
            ...
            for (int i = 0; i < imageIn->x; i++)
            {
                ...
                line_buffer[i].blue +=
                    imageIn->data[numberOfValuesInEachRow * endy + i].blue;
                line_buffer[i].red +=
                    imageIn->data[numberOfValuesInEachRow * endy + i].red;
                line_buffer[i].green +=
                    imageIn->data[numberOfValuesInEachRow * endy + i].green;
            }
        }
        ...
        else if (endy >= imageIn->y)
        {
            ...
            {
                line_buffer[i].blue -=
                    imageIn->data[numberOfValuesInEachRow * (starty - 1) + i]
                        .blue;
                line_buffer[i].red -=
                    imageIn->data[numberOfValuesInEachRow * (starty - 1) + i]
                        .red;
        ...
        {
            // in this loop, we do exactly the same thing as before but only
            // with the array line_buffer
            ...
            {
                sum_red +=
                    (line_buffer[endx].red - line_buffer[startx - 1].red);
                sum_green +=
                    (line_buffer[endx].green - line_buffer[startx - 1].green);
                sum_blue +=
                    (line_buffer[endx].blue - line_buffer[startx - 1].blue);
            }
...
void
performNewIdeaFinalization(AccurateImage* imageInSmall,
